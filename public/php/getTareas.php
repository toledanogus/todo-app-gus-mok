...
if ($x->filter === 'todas') {
    ...
    $respuesta = mysqli_query($conn, "(SELECT titulo, descripcion, prioridad, completada, STR_TO_DATE(fechalimite, '%Y-%m-%d') AS fechalimite, categoria, id FROM tareasgus) UNION ALL (SELECT titulo, descripcion, prioridad, completada, STR_TO_DATE(fechalimite, '%Y-%m-%d') AS fechalimite, categoria, id FROM tareasjuntos) ORDER BY fechalimite ASC, prioridad DESC");
